...
void drawCurve(int type)
{
	...
	while (theta <= twoPi)
	{
		...
		switch (type) 
		{
		case LICK_MY_ASS: r = a * cos(theta) + b; break; //lemacon
		case CANCER_PATIENT: r = a * (1 + cos(theta)); break; // cardio
		case WEED: r = a * cos(3 * theta); break;	// leaf
		case TOO_HIGH: r = (a / 4.0) * theta; break; //spiral
		default: break;
		}
